  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (totalSameDeviceBorrowed + quantity > 2 || totalAllDeviceBorrowed + quantity > 3) {
        dispatch(setAlert('Gửi thất bại, bạn đã đến giới hạn mượn', 'error'));
        setLoading(false);
        return;
      }
      await dispatch(
      ...
      {equipment.availableQuantity === 0 ? (
        ...
        >
          {/* <Alert
            ...
            description={
              <div>
                <p>• Tổng số thiết bị đang mượn: {totalAllDeviceBorrowed}/3</p>
                <p>• Tổng số thiết bị cùng loại đang mượn: {totalSameDeviceBorrowed}/2</p>
              </div>
            }
            type="info"
            showIcon
